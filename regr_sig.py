...
options = SET(nsim=1000, method='ttest', alpha=0.05)

debug = False


def regr_sig(x, y, options=options):
    ...
    if options.method == 'ttest':
        (regcoef, signif, p) = regr_ttest(x, y, alpha=options.alpha)
    elif options.method == 'isopersistent':
        (regcoef, signif, p) = regr_isopersist(x, y, alpha=options.alpha, nsim=options.nsim)
    elif options.method == 'isospectral':
        (regcoef, signif, p) = regr_isospec(x, y, alpha=options.alpha, nsim=options.nsim)

    return regcoef, signif, p


def regr_ttest(x, y, alpha=0.05):
    ...
    """
    reg = LinearRegression().fit( y[:, None], x)
    regcoef = reg.coef_[0]
    r = pearsonr(x, y)[0]
    ...
    return regcoef, signif, pval


def regr_isopersist(x, y, alpha=0.05, nsim=1000):
...
def ar1_fit(ts):
    ...
    '''
    #ar1_mod = sm.tsa.arima_model.ARMA(ts, (1, 0)).fit()
    #g = ar1_mod.params[1]
    g = auto_corrcoef(ts)   
...
def regr_isospec(x, y, alpha=0.05, nsim=1000):
...
def IndexRegression(Data,Index,alfa):
    [n1,n2]=Data.shape #los datos introducidos en Data tienen que tener dimesión n1=espacio (o número de modos), n2=tiempo 
    ...
    for nn in range(n1): #para cada punto del espacio hacemos la correlación de Pearson
        bb=pearsonr(Data[nn,:],Index) #bb tiene dos salidas: la primera es corre y la segunda es p-value que es el nivel de confianza
...
def r_statsig(x,y,alpha):
    """ Calcula el coeficiente de correlación correspondiente a la significación estadísitca marcada en "alpha" con un two-tailed t-test. 
    Los degrees of freedom se calculan teniendo en cuenta la autocorelación de lag 1 de las series x e y """

    # Quito los NaNs para hacer las lead-lag regressions""
    ...
    # Tengo dos raices la positiva y la negativa

    r1 = np.sqrt(t_stat**2 / (df + t_stat**2))

    return r1


def lead_lag_corr(ds_1, var_1, ds_2, var_2, lags, alpha):
    """ Función para calcular las lead-lag correlations entre las variables var_1 y var_2 de los datasets ds_1 y ds_2"""
    import pandas as pd
    import xarray as xr
    import numpy as np
    import sys
    #sys.path.append('/home/pfernand/Postdoc/Python_scripts/')
    #from regr_sig import r_statsig
    
    
    r_1_2 = xr.concat([xr.corr(ds_1[var_1], ds_2[var_2].shift(year = lag), dim = 'year') for lag in lags], pd.Index(lags, name="lag"))

    # Calculo el coeficiente de correlación estadísticamente signifcativo para un valor de alpha
    r_1_2_alpha = xr.concat([xr.apply_ufunc(
        r_statsig, # First the function
        ds_1[var_1],
        ds_2[var_2].shift(year = lag), 
        alpha,
        input_core_dims = [["year"],["year"], []], 
        output_core_dims = [[]], 
        exclude_dims = set(()), 
        vectorize = True, 
        ) for lag in lags], pd.Index(lags, name="lag"))

    # Aplico una máscara a los coeficientes que no sean significativos 

    r_1_2_sig = xr.where(np.abs(r_1_2) < r_1_2_alpha, np.nan, r_1_2)

    return r_1_2, r_1_2_alpha, r_1_2_sig
